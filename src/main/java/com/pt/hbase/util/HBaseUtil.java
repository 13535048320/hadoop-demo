...
public class HBaseUtil {
    private Logger logger = LoggerFactory.getLogger(HBaseUtil.class);
    ...
    public boolean createNamespace(String namespace) {
        ...
        try {
            ...
            e.printStackTrace();
            logger.error("创建命名空间{}失败", namespace);
            return false;
        } finally {
     ...
     * @return
     */
    public Map<String, Map<String, String>> scanByRowKey(String tableName, String rowKey, CompareOperator operator) {
        Scan scan = new Scan();
        Filter filter = new RowFilter(operator, new BinaryComparator(rowKey.getBytes()));
        scan.setFilter(filter);
        return this.queryData(tableName, scan);
    }
     ...
     * @return
     */
    public Map<String, Map<String, String>> scanByFilter(String tableName,FilterList filterList) {
        Scan scan = new Scan();
